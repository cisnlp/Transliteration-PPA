...
import argparse
import os
from transformers import BertTokenizer, RobertaTokenizer, XLMRobertaTokenizer, XLMTokenizer
...
def tokenize_preprocess(args):
    def _preprocess_one_file(infile, outfile, idxfile, tokenizer, max_seq_len):
        ...
        special_tokens_count = 3 if isinstance(tokenizer, XLMRobertaTokenizer) or isinstance(tokenizer,
                                                                                             RobertaTokenizer) else 2
...
def main():
    ...
    parser.add_argument('--min_checkpoint_num', type=int, default=0)

    args = parser.parse_args()
    args.predict_langs = ['eng_Latn', 'tur_Latn', 'mlt_Latn', 'fas_Arab', 'heb_Hebr',
                          'amh_Ethi', 'arz_Arab', 'ara_Arab', 'ell_Grek', 'bod_Tibt',
                          'wuu_Hani', 'mya_Mymr', 'lzh_Hani', 'tha_Thai', 'yue_Hani', 'zho_Hani',
                          'kor_Hang']
    # with open('/dss/dsshome1/06/di35ver/evaluation/tagging/ner_lang_list.txt', 'r') as f:
    #    lines = f.readlines()
    #    for line in lines:
    ...
    checkpoints = [checkpoint for checkpoint in checkpoints
                   if args.min_checkpoint_num <= int(checkpoint.split('-')[1]) <= args.max_checkpoint_num]
    ...
    run_combos = [(checkpoint, lang, seed_nr)
                  for checkpoint in checkpoints
                  for lang in args.train_langs.split(',')
                  for seed_nr in range(args.nr_of_seeds)]
    default_seed = args.seed
    for checkpoint, lang, seed_nr in run_combos:
        print(f"Running {checkpoint} on {lang} with seed {seed_nr}")
        seed = default_seed + seed_nr

        args.output_dir = main_output_dir + f"/{checkpoint}_{lang}_{seed}"
        args.train_langs = lang
        args.seed = seed
