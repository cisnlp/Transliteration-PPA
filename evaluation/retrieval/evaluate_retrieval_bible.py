    ...
    with open(os.path.join(args.output_dir, f'test_results.txt'), 'w') as result_writer:
        for src_lang in args.predict_langs:
            ...
            src_text_file = os.path.join(
                data_dir, 'bible.{}-{}.{}'.format(src_lang, 'eng_Latn', src_lang))
            ...
            if tgt_lang == 'eng_Latn':
                ...
                tgt_text_file = os.path.join(
                    data_dir, 'bible.{}-{}.{}'.format(tgt_lang, 'eng_Latn', tgt_lang))
...
def main():
    ...
    args.predict_langs = []
    with open('/dss/dsshome1/06/di35ver/evaluation/retrieval/bible_lang_list.txt', 'r') as f:
        lines = f.readlines()
        for line in lines:
    ...
    run_combos = [(checkpoint, lang) for checkpoint in checkpoints for lang in args.src_language.split(',')]
    for checkpoint, lang in run_combos:
        print(f"Current checkpoint: {checkpoint}...")
        args.output_dir = main_out_dir + f"/{main_model_dir.split('/')[-1]}"
        args.output_dir += f"/{checkpoint}_{lang}"
