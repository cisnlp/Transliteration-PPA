...
def read_data(file):
    ...
    df['label'].replace({
        'Recommendation': int(0), 'Faith': int(1), 'Violence': int(2),
        'Grace'         : int(3), 'Sin': int(4), 'Description': int(5)
    }, inplace=True)
...
def run(args):
    ...
    if args.do_train:
        train_file = f"{args.train_data_dir}/860_{args.source_language}_train.tsv"
        ...
        dev_file = f"{args.train_data_dir}/{args.source_language}_dev.tsv"
...
def main():
    ...
    run_combos = [(checkpoint, lang, seed_nr)
                  ...
                  for lang in args.source_language.split(',')
